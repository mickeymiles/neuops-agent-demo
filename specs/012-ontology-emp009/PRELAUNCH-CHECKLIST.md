...
1. **在 9006 页面配置参与方（新增，务必先做）**
   智能体的供应商与审批人已改为**直读 9006 库**（`procurement_supplier` / `procurement_approver`），不再用 `ONT_SUPPLIERS` / `ONT_APPROVERS` 环境变量。
   - **供应商**：确认「供应商」页至少 2 家（当前 2 家：华为总代 星辰设备 / 智联信息科技）。
   - **审批人**：「审批人」页**当前为空，必须新增**（如 李审批 `biquanzhi5@163.com`）。
     不配的话 `orbit.py:406` 因 `approver_emails` 为空，**审批回复永远不会被识别、流程卡在待审批**；代码会打 WARNING 并回退旧 env 链路，但上线不应依赖回退。
   - **抄送（系统配置）**：「抄送」页当前 2 人，见下方数据风险。
   - **邮件模板（可选）**：「邮件模板」页可改 A–G 措辞；留空即用系统默认。
2. **部署新版（最关键）** — 线上旧代码无 `settlement_enabled`，工程师发"更换完成"会**真的发出 G 结算邮件**，与"暂不需要结算"冲突。部署新版后默认关闭。
   > 需一并部署上述「页面配置」相关改动（9007 读库 + 9006 页面），否则页面改了智能体读不到。
3. **提交并推送两仓库本地改动** — ✅ 已完成（9007 `afbc2fb` + `0bfac41`、9006 `959a2d3` + `e61865e`）。
4. **清场** — 停 009 → 清本体库八张表 → 清 6 个 163 邮箱 → 起 009。当前库内有 1 个残留任务/邮件，不清会混进试运行数据、难以判读结果。
5. **确认决策模式**（规则 vs LLM）— 见 §3。

### ⚠️ 数据风险：全局抄送里混入了供应商邮箱

「抄送」页当前为 `rich-miles@163.com`、`biquanzhi@163.com`，其中 **`biquanzhi@163.com` 同时是供应商「智联信息科技」的邮箱**。
抄送是"系统配置 + 首封 A 抄送"双路叠加，该供应商因此会被抄送到**发给其他供应商**的询价/订货邮件，存在**报价泄露**风险。
建议：上线前把它从全局抄送移除（若需其旁观，改由工程师在首封邮件 A 里单独抄送）。

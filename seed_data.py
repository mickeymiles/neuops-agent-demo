...
MOCK_EMPLOYEES = [
    ...
    {"id": "emp-004", "name": "经营业务分析专家", "desc": "专注采购合同比对、经营指标分析、合同明细探查等经营业务场景。对接9006经营分析系统：通过原子本体MCP查询原始合同/付款/收款明细，通过指标数据集MCP查询签单毛利率等定时ETL预计算指标，通过合同比对引擎分析供应商报价差异。", "type": "经营分析", "created": "2026-08-11", "updated": "2026-08-13",
     ...
     "rag_kb": "经营知识库-合同案例", "prompt": "你是一位经营业务分析专家，负责经营分析和顾问工作。你通过9006经营分析系统的三类能力服务用户：①合同比对——用户在9006上传合同基准Excel和供应商报价Excel后，查询比对结果、分析差异、识别高危异常项；②指标分析——通过指标数据集MCP读取定时ETL预计算的签单毛利率、回款毛利率等指标宽表，做同比/环比解读，不做原始聚合计算；③明细探查——通过原子本体MCP按合同编号或关键词查询原始合同、付款、收款明细；④本体计算——当用户给出具体参数、要求计算回款周期/资金占用/毛利率/ROI/成本预警等指标时，必须调用「本体计算」工具（ontology_compute）。红线（重要）：禁止你自行心算或凭空编造任何指标数值，那是口径漂移与幻觉的来源，需要数字时必须调用工具——查已有报表用「指标数据集MCP」（定时ETL预计算宽表，用于同比/环比解读），按给定参数做本体指标计算用「本体计算」工具（ontology_compute，与9006权威计算面调用同一批ontos纯函数，输入相同则结果一致，属同源权威口径）。预计算宽表已覆盖的指标以ETL口径为准；ETL未覆盖或用户给了具体参数要算的，用本体计算工具。", "model": "deepseek-v4"},
    ...
    {"id": "emp-006", "name": "项目管理成本利润治理专家", "desc": "承接公司两单一物体系，事业部精细化项目过程治理与集团口径指标监控：里程碑跟进、日报工时合规治理、四算刚性约束监控（概算≥预算≥核算≥决算）、集团考核指标（人均效/元效/双按完成率）、工时→成本→利润闭环联动，全部只读研判+预警，变更由人工执行。", "type": "项目治理", "created": "2026-08-16", "updated": "2026-08-16",
     "skills": ["skill-20"],
     "rag_kb": "项目知识库-四算与集团指标", "prompt": "你是一位项目管理与成本利润治理专家，负责事业部精细化项目过程治理与集团口径指标监控，全部只读研判+预警，所有变更由人工执行。你的能力：①项目全生命周期管控——通过pm_project_read查询项目基础信息/里程碑/四算数据（概算/预算/核算/决算），识别里程碑逾期、任务积压、成员负载；②日报工时合规治理——通过pm_workhour_read查询日报/工时明细，识别敷衍、空填、溢出、少填、堆填等异常，输出每日整改清单与部门合规率；③四算刚性约束监控——自动校验概算≥预算≥核算≥决算，识别超概算/超预算/超核算的集团风险项目，输出四算对比台账与偏差分析；④集团考核指标监控——通过biz_metric_read读取人均效/元效/双按完成率（按期完成率、按预算完成率）预计算指标，跟踪事业部月度集团指标达成情况；⑤工时→成本→利润闭环——通过pm_cost_calc按日报工时折算人力成本，联动合同目标利润复盘真实利润率；⑥两单一物对齐校验——通过pm_task_read校验工单、任务、工时三者一致性。红线：所有能力只读研判与预警，绝不执行任何写操作或业务变更。【数据红线·必须遵守】①pm_project_read 返回的是本体真实数据（项目主数据 md_contract，project_id 即合同编号），成本判定以本体 F-project-cost-warning 为准；②四算(概算/预算/核算/决算)、里程碑、工时/日报、工单/任务当前【均未接入】，工具会返回 not_available——此时必须如实告知用户“该数据尚未接入”，严禁估算、严禁编造项目或数字；③biz_metric_read 的集团指标是【演示数据】，引用时必须标注为演示口径；④任何情况下都不得虚构项目号（如 P-2026-xxx 之类）或成本数字。【分页红线·极易出错】pm_project_read / pm_cost_calc 默认只返回前 20 条，响应里的 total（筛选后全量）、total_all（全库）、status_count（全库分布）才是全集统计。truncated=true 时必须带 offset=next_offset 继续翻页，或按 status=超支/预警/正常 筛选后遍历。严禁把某一页的条目数当成项目总数（反例：只看到 19 条就回答共有 19 个项目，实际全库 629 个）。汇报共有多少项目/多少超支，一律以 total_all 与 status_count 为准。", "model": "deepseek-v4"},
...
SKILL_DETAILS = {
    ...
    "skill-11": {
        "type": "Workflow业务编排技能",
        "prompt": "你是经营指标分析专家，负责分析9006经营分析系统的经营指标。\n"
                  "1、查历史指标趋势：通过指标数据集MCP读取定时ETL预计算的签单毛利率、回款毛利率等指标宽表，做同比/环比解读，识别波动与异常；\n"
                  "2、按给定参数计算本体指标：当用户给出具体参数、要求计算 回款周期/资金占用/毛利率/ROI/成本聚合/应收状态/成本预警 时，"
                  "调用「本体计算」工具（ontology_compute）。它与9006权威计算面调用同一批 ontos 纯函数，输入相同则结果一致，属同源权威口径；\n"
                  "3、红线（重要）：禁止你自行心算或凭空编造任何指标数值，那是口径漂移与幻觉的来源——需要数字时必须调用工具。"
                  "宽表已覆盖的指标以ETL口径为准；宽表未覆盖、或用户给了具体参数要算的，用本体计算工具；\n"
                  "4、输出经营指标分析报告，并注明每个数字的数据来源（ETL预计算宽表 / 本体计算）。",
        "tools": ["ontology_compute", "get_etl_metrics", "get_metrics"],
        "flow": "执行流程：\n"
                "1、判断需求类型：查历史趋势/同比环比 → 走指标数据集MCP；按给定参数算本体指标 → 走「本体计算」工具\n"
                "2、调用相应的工具取得数据（禁止心算）\n"
                "3、做同比/环比或口径分析，识别波动与异常\n"
                "4、输出经营指标分析报告，注明数据来源与口径",
    },
    "skill-12": {
        "type": "Workflow业务编排技能",
...
MCP_TOOL_SEED = [
    ...
    {"id": "pm_project_read", "name": "项目档案与成本执行", "desc": "查询项目档案（部门/责任人/区域/合同状态/签约额）与预算/成本执行，"
     "含本体成本预警判定（正常/预警/超支）。数据源=共享本体 ontos（项目主数据 md_contract），"
     "与 9006 成本预警页同源。project_id 即合同编号（如 DFSY1410017C）。"
     "★分页语义：不带 project_id 时只返回前 limit 条（默认20），但 total/total_all/"
     "status_count 均为【全量统计】；truncated=true 表示被截断，须带 offset=next_offset "
     "继续翻页，或按 status=超支/预警/正常 筛选后遍历。"
     "【禁止】把本页条目当全集回答共有 X 个项目——总数一律看 total_all。"
     "⌛四算（概算/预算/核算/决算）与里程碑尚未接入，返回 not_available", "icon": "📐", "tag": "只读查询", "danger": 0, "category": "项目管理",
     "method": "POST", "path": "/tools/pm_project_read",
     "params_schema": [
         {"name": "project_id", "type": "string", "required": False, "desc": "项目ID（=合同编号，如 DFSY1410017C），为空返回列表"},
         {"name": "status", "type": "string", "required": False, "desc": "按本体成本判定筛选：超支/预警/正常"},
         {"name": "limit", "type": "number", "required": False, "desc": "返回条数上限，默认20（不影响 total 统计）"},
         {"name": "offset", "type": "number", "required": False, "desc": "翻页偏移量，取上一页返回的 next_offset"},
     ]},
    {"id": "pm_task_read", "name": "两单一物工单任务", "desc": "查询两单一物工单、任务明细与状态。⌛数据源未接入（plm_task/plm_assignment 为空表），"
     "调用后返回 not_available——须如实告知用户该数据尚未接入，不得估算或编造", "icon": "📋", "tag": "只读查询", "danger": 0, "category": "项目管理",
     "method": "POST", "path": "/tools/pm_task_read",
     "params_schema": [
         {"name": "project_id", "type": "string", "required": False, "desc": "项目ID（=合同编号）"},
         {"name": "status", "type": "string", "required": False, "desc": "任务状态：pending/running/done/overdue"},
     ]},
    {"id": "pm_workhour_read", "name": "日报工时明细", "desc": "查询日报、工时明细与人员填报数据。⌛数据源未接入（主数据无工时列、PMO 域未建设），"
     "调用后返回 not_available——须如实告知用户该数据尚未接入，不得估算或编造合规率", "icon": "🕐", "tag": "只读查询", "danger": 0, "category": "项目管理",
    ...
    {"id": "pm_cost_calc", "name": "项目成本明细", "desc": "项目成本明细：预算三分量（硬件集成费/服务预估成本/软件预估实施费）与"
     "成本六分量（硬件集成费实际/软件实际实施费/往年·当年实际服务直接·间接）+ 本体预警判定。"
     "数据源=共享本体 ontos（md_contract），与 9006 成本预警同源。"
     "注：人力成本折算依赖工时数据，当前 ⌛未接入", "icon": "💰", "tag": "只读查询", "danger": 0, "category": "项目管理",
     "method": "POST", "path": "/tools/pm_cost_calc",
     "params_schema": [
         {"name": "project_id", "type": "string", "required": False, "desc": "项目ID（=合同编号），为空返回前20条"},
         {"name": "limit", "type": "number", "required": False, "desc": "返回条数上限，默认20"},
     ]},
    {"id": "biz_metric_read", "name": "经营集团指标", "desc": "读取预计算经营&项目集团指标（人均效/元效/双按完成率/四算偏差）。"
     "⚠ 该指标为演示数据（非本体真实数据），未接入本体，引用时必须标注为演示口径", "icon": "📊", "tag": "只读查询", "danger": 0, "category": "项目管理",
    ...
    # =====================================================================
    # 本体计算类（直调共享 ontos 子模块，与 9006 固化显示同一份算法 → 同源权威口径）
    #
    # ★措辞说明（重要）：此处的用途是「探索/模拟/预测/假设分析」，但**算法与
    #   9006 固化报表同源**——调用的是同一批 ontos 纯函数，结果必然一致。
    #   早期版本把 name/desc 写成「临时口径…非固化口径」，本意指「用途非固化
    #   报表」，却被 LLM 理解成「结果不权威」，导致智能体宁可去查 ETL 宽表也
    #   不用本体计算（实测：问 ROI 却答毛利率）。故改为「同源权威口径」。
    # =====================================================================
    {"id": "ontology_compute", "name": "本体计算（与9006同源权威口径）", "icon": "🧮", "tag": "只读计算", "danger": 0, "category": "本体",
     "desc": "调用本体 TBox 纯函数计算回款周期/资金占用/毛利率/ROI/成本聚合/应收状态/成本预警。"
             "★算法与 9006 固化报表同源：本工具与 9006 权威计算面调用同一批 ontos 纯函数，"
             "输入相同则结果必然一致，是同源权威口径。"
             "用途上的「探索」指可做模拟与假设分析（如调整参数看 ROI 变化），不代表结果不权威。"
             "需要按给定参数做本体指标计算时应优先使用本工具，而非自己推算。",
     ...
     "params_schema": [
         ...
         {"name": "params", "type": "object", "required": False,
          "desc": "参数字典。**必须使用下列参数名**（写错会报 param_error）：\n"
                  "- payment_cycle(回款周期): {sign_date, receipts:[{received_date,amount}], basis:'last'(默认)|'first', recv_source}\n"
                  "- capital_occupation(资金占用): {payments:[{date,amount}], receipts:[{date,amount}]}\n"
                  "- project_margin(毛利率): {sign_amount, sign_gross_profit}\n"
                  "- project_roi(ROI): {revenue, current_cost}   ← 注意不是 contract_amount/cost\n"
                  "- cost_rollup(成本聚合): {payments, cost_detail_rows}\n"
                  "- receivable_status(应收状态): {invoice_date, due_date, amount, received_amount, received_date, today}\n"
                  "- project_cost_warning(成本预警): {estimate, budget, current_cost}\n"
                  "金额用元为单位的数字；日期用 'YYYY-MM-DD' 字符串。"},
     ]},
]

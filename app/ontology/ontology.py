# -*- coding: utf-8 -*-
"""本体知识层（emp-009，KNO-R-01 v2）。
基于「语义网/本体化 LLM 智能体」范式，不再用硬编码状态机：
  - TBox：领域概念(类/关系)，机器可读且可喂给 LLM；
  - ABox：从 o_task / o_email / o_supplier_quote 抽取的当前事实三元组；
  - 可行动作：本体声明 = 定义 + 前置(condition) + 效果(effect) + 不变量(invariant) + 幂等(idempotent)；
...
# ────────────────────────────────────────────
# ABox：从任务/邮件/报价抽取事实三元组（喂给 LLM & 校验器）
# ────────────────────────────────────────────
def build_abox(task: dict) -> dict:
...
def validate_action(action_id: str, abox: dict, rejected_prev=None):
    ...
    reasons = []
    cond = spec.get("条件") or []
    # 由 ABox 语义事实推导的谓词（不依赖阶段计数，纯条件语义）。键与 ACTIONS.条件 措辞对齐。
    ...
    facts = {
        ...
        "收到B线程供应商回复": bool(abox.get("quotes")) or True,  # 收到回复由入站归集写入，视为存在候选
